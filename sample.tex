...
\end{tikzpicture}
\ref{defn:chuoi-htpk}
\hyperlink{chproof}{Xem Chứng minh ở Chương 2}.
...
\begin{problem}[Phần 3]%
  ...
  \begin{enumerate}
    \item $\sum_{n=1}^{+\infty}\frac{n^{2}}{\alpha^{n}}$ $\alpha>1$
      \AnswerMark{HT nếu $\alpha>1$; PK nếu $\alpha\le1$}
      \begin{solution}
        asdasd
      \end{solution}
...
\begin{problem}
  ...
  \begin{equation}
    \int_{1}^{\infty} f(x)dx
  \end{equation}\ExampleMark{}\QuickMark{}
  \begin{solution}
    solution
